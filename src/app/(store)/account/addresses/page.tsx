...
export default async function AddressesPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-lg font-extrabold text-slate-900">آدرس‌های من</h1>
          ...
          <div key={a.id} className="flex items-start justify-between gap-3 rounded-2xl border border-slate-100 p-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                {a.isDefault && <span className="rounded-full bg-brand-50 px-2 py-0.5 text-[11px] font-bold text-brand-600">پیش‌فرض</span>}
              </div>
              <p className="mt-1 text-xs text-slate-500">{a.province}، {a.city}، {a.addressLine}</p>
              <p className="mt-1 text-xs text-slate-400">کد پستی: {a.postalCode} | تماس: {a.phone}</p>
            </div>
