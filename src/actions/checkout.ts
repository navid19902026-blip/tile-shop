...
export async function placeOrder(input: z.infer<typeof checkoutSchema>): Promise<CheckoutState> {
  ...
  if (data.discountCode) {
    ...
    if (!code || !code.isActive || (code.expiresAt && code.expiresAt < new Date())) {
      return { error: "کد تخفیف نامعتبر یا منقضی‌شده است" };
    }
    if (code.maxUses && code.usedCount >= code.maxUses) return { error: "ظرفیت استفاده از این کد تخفیف تمام شده است" };
    if (subtotal < code.minOrderAmount) return { error: `حداقل مبلغ سفارش برای این کد ${code.minOrderAmount.toLocaleString()} تومان است` };
    ...
    if (pointsUsed > 0) {
      ...
      await tx.loyaltyTransaction.create({
        data: {
          ...
          points: pointsUsed,
          orderId: created.id,
          description: `استفاده از امتیاز برای سفارش #${created.id.slice(-6)}`,
        },
      });
    }
